/// General notes:
///
/// - It is unspecified how overlapping words are managed. For example, the line
///
/// ```
/// 2dpzvd39eightwohjc
/// ```
///
/// presumably should result in the output `22`, but with non-overlapping parsing from
/// left to right, it would be `28`
///
use gumdrop::Options;
use nom::{
    branch::alt,
...
struct Args {
    // Also count number words
    words: bool,

    // Allow overlapping words to be parsed multiple times
    overlapping: bool,
}

// Convert an ASCII digit to it's numeric value.
//
// # Panics
//
// May panic if not given an ASCII digit.
fn ascii_digit_to_value(character: char) -> u64 {
    // By construction, ascii digits fall in the range 0x30 - 0x39
    (character as u64) - 0x30
}

fn integer_word_to_value(word: &str) -> u64 {
    match word {
        "one" => 1,
        "two" => 2,
        "three" => 3,
...
fn digit_value(input: &str) -> IResult<&str, u64> {
    combinator::map(
        take_while_m_n(1, 1, |c: char| c.is_ascii_digit()),
        |c: &str| ascii_digit_to_value(c.chars().nth(0).expect("one character") as char),
    )(input)
}

fn word_value(input: &str) -> IResult<&str, u64> {
    let word = alt((
        tag("one"),
        tag("two"),
        tag("three"),
    ...
    combinator::map(word, integer_word_to_value)(input)
}

fn other_value(input: &str) -> IResult<&str, u64> {
    combinator::map(take(1usize), |_| 0)(input)
}

/// Given an input string, parses the next value in the input.
///
/// Either a digit, word, or an irrelevant character.
fn item(input: &str) -> IResult<&str, u64> {
    alt((digit_value, word_value, other_value))(input)
}

fn find_digits_or_words(line: &str) -> (u64, u64) {
    let mut all = all_consuming(many0(item));
    let (_remaining, values): (&str, Vec<u64>) = all(line).expect("Error parsing line");
    ...
    (first.expect("No ascii number in line"), last)
}

fn starts_with_value(string: &str) -> u64 {
    let (_remaining, value) = item(string).expect("Error parsing line");
    value
}

fn find_digits_or_words_overlapping(line: &str) -> (u64, u64) {
    let mut first = None;
    let mut last = 0;
    for (index, _char) in line.char_indices() {
        let value = starts_with_value(&line[index..]);
        if value == 0 {
            continue;
        };
...
fn find_digits(line: &str) -> (u64, u64) {
    let mut first = None;
    let mut last = 0;

    for character in line.chars() {
        if character.is_ascii_digit() {
            let value = ascii_digit_to_value(character);
            if first == None {
                first = Some(value);
            }
            last = value;
...
fn main() {
    ...
    for line in INPUT.lines() {
        let (first, last) = if opts.words {
            if opts.overlapping {
                find_digits_or_words_overlapping(line)
            } else {
                find_digits_or_words(line)
            }
        } else {
            find_digits(line)
        };
